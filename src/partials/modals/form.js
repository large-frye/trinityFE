...
(function () {
    ...
    angular
        .module('trinity.controllers.inspections.inspector.form', [])
        .controller('inspector.formCtrl', InspectionController);

    InspectionController.$inject = ['InspectionService', 'InspectionFactory', 'FormService', '$log', 'STATUSES', 'inspection', '$rootScope', '$modal',
        '$routeParams', 'shared', 'alert', 'FORM', 'UserFactory', 'FileService', '$interval', 'USER_TYPES'];

    function InspectionController(InspectionService, InspectionFactory, FormService, $log, STATUSES, inspection, $rootScope, $modal
        , $routeParams, shared, alert, FORM, UserFactory, FileService, $interval, USER_TYPES) {
        var vm = this;
        ...
        vm.isInspectionBillingDetailsLocked = isInspectionBillingDetailsLocked;
        ...
        function activate() {
            getWorkorder(function () {
                isInspectionBillingDetailsLocked();
        ...
        function setForm() {
            if (inspection.inspection) {
                vm.form = {};
                var inspectionItems = inspection.inspection;
        ...
        function isInspectionBillingDetailsLocked() {
            vm.billingLocked = vm.workorder.billing_locked;

            var interval = setInterval(function() {
                InspectionService.getWorkorderBillingLocked({
                    id: vm.workorder.id
                }, function(data) {
                    vm.billingLocked = data.status;
                }, function(err) {
                    $log.error(err);
                });
            }, 30000);
